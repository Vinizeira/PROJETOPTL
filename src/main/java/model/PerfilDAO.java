...
public class PerfilDAO extends DataBaseDAO {
    ...
    public ArrayList<Perfil> getLista() throws Exception {
        ...
        while(rs.next()){
            Perfil p = new Perfil();
            p.setIdPerfil(rs.getInt("idPerfil")); //Paramento do BANCO
    ...
    public boolean gravar (Perfil p){
        try {
         ...
         this.conectar();
         if(p.getIdPerfil()==0){
             sql = "INSERT INTO perfil(nome) VALUES(?)";//Inserindo no BANCO
          } else {
             sql="UPDATE perfil SET nome=? WHERE idPerfil=?";
         }
           
            PreparedStatement pstm = conn.prepareStatement(sql);
            pstm.setString(1, p.getNome()); //SET CASO SEJA CADASTRO NAS INFORMAÇÕES
              
            if(p.getIdPerfil()>0){
                    pstm.setInt(2, p.getIdPerfil());//CONDIÇÃO CASO SEJA UPDATE
                }
                pstm.execute();
                this.desconectar();
                return true;
    ...
    public Perfil getCarregaPorID(int idPerfil) throws Exception{
        ...
        if (rs.next()){
            p.setIdPerfil(rs.getInt("idPerfil")); //Set nos valores direto do banco
            p.setNome(rs.getString("nome"));
